        ...
        while priority_queue:
            ...
            for node_id, distance in sorted_distances:
                if node_id != start_node.id:
                    ...
                    shortest_edges.append((node.id, node.get_name(), node.get_quantity(), node.get_brand(), node.get_category(), node.get_price(), node.get_url(), distance))
...
def index():
    ...
    graph = build_graph_from_csv('csv_Test.csv')
...
def recomendacion(producto_id):
    ...
    if producto_seleccionado:
        ...
        recomendaciones = [(node_id, node_name, node_quantity, node_brand, node_category, node_price, node_url, distance) for node_id, node_name, node_quantity, node_brand, node_category, node_price, node_url, distance in shortest_edges]
        image_base64 = print_graph(producto_seleccionado, recomendaciones)  # Llamada a la función print_graph
        return render_template('recomendation.html', producto_seleccionado=producto_seleccionado, recomendaciones=recomendaciones, graph_image=image_base64)
    else:
        return "Producto no encontrado"

def get_producto_por_id(producto_id):
    with open('csv_Test.csv', 'r', newline='', encoding='utf-8') as csv_handle:
        csv_products = csv.reader(csv_handle, delimiter='@')
        for line in csv_products:
            if len(line) >= 7:
